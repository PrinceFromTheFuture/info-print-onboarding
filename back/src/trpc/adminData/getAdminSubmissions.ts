...
const getAdminSubmissions = privateProcedure.query(async ({ ctx }) => {
  ...
  const allAssignmentQuestionIds: string[] = [];

  allActiveAssignments.docs.forEach((assignment) => {
    if (typeof assignment.template !== "string") {
      const template = assignment.template as Template;
      const appUser = typeof assignment.appUser !== "string" ? (assignment.appUser as AppUser) : null;
  ...
  const assignedTemplatesData = await Promise.all(
    allAssignments.docs.map(async (assignment) => {
      const appUser = typeof assignment.appUser !== "string" ? (assignment.appUser as AppUser) : null;
      const template = typeof assignment.template !== "string" ? (assignment.template as Template) : null;
      ...
      let lastUpdatedSubmission: string | null = null;
