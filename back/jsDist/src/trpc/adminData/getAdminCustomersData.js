...
const getAdminCustomersData = privateProcedure.query(async ({ ctx }) => {
    ...
    const assignmentsThisWeek = assignmentsLastTwoWeeks.docs.filter((assignment) => dayjs(assignment.updatedAt || assignment.createdAt).isSame(dayjs(), "week"));
    const assignmentsLastWeek = assignmentsLastTwoWeeks.docs.filter((assignment) => dayjs(assignment.updatedAt || assignment.createdAt).isSame(dayjs().subtract(1, "week"), "week"));
