...
const getAdminDashboardData = privateProcedure.query(async ({ ctx }) => {
    ...
    const recentSubmittedAssignmentsAvctivity = recentSubmittedAssignments.docs.map((assignment) => ({
    ...
    const last5UsersCreatedAvctivity = last5UsersCreated.docs.map((user) => ({
        name: user.name,
        email: user.email,
        createdAt: user.createdAt,
        assignments: last5UsersCreatedAssinmnets.docs.filter((assignment) => assignment.appUser === user.id || (typeof assignment.appUser === "object" && assignment.appUser?.id === user.id)).length,
    }));
    return {
