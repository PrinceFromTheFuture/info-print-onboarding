...
export default function FormPage({ params }: { params: Promise<{ formId: string }> }) {
  ...
  useEffect(() => {
    if (unwrappedParams.formId) {
      dispatch(getFilledTemplateByIdAsyncThunk(unwrappedParams.formId));
    }
      ...
      <div className="flex items-center justify-center h-screen">
        <Card className="p-6 max-w-md">
          <div className="text-center">
            ...
            <Button onClick={() => dispatch(getFilledTemplateByIdAsyncThunk(unwrappedParams.formId))}>Try Again</Button>
          </div>
        </Card>
      </div>
    );
